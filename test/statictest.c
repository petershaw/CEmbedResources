...
void testMulitigetDifferentElementByResource() {
    CER_resourcetable_t *table = malloc(sizeof (CER_resourcetable_t));
    CEmbedResources_Init(table, &initialize_resources);
    ...
    for (i = 0; i < 10000; i++) {
        ...
        // get a
        CER_resource_t *result01 = malloc(sizeof (CER_resource_t));
        int depth01 = CEmbedResources_GetResourceByIdentifyer(table, (char *) "ADAD0000000000000000000000000000", result01);
        printf("-- %s (%d) len: %d\n", result01->resourceName, depth01, strlen(result01->content));
        int len01 = strlen(result01->content);
        free(result01);

        // get b
        CER_resource_t *result02 = malloc(sizeof (CER_resource_t));
        int depth02 = CEmbedResources_GetResourceByIdentifyer(table, (char *) "D6670C850CD9357B2EE617E4C21CC5EB", result02);
        printf("-- %s (%d) len: %d\n", result02->resourceName, depth02, strlen(result02->content));
        int len02 = strlen(result02->content);
        free(result02);

        CU_ASSERT_FALSE(len01 == len02);

        // get a again, 
        CER_resource_t *result03 = malloc(sizeof (CER_resource_t));
        int depth03 = CEmbedResources_GetResourceByIdentifyer(table, (char *) "ADAD0000000000000000000000000000", result03);
        printf("-- %s (%d) len: %d\n", result03->resourceName, depth03, strlen(result03->content));
        int len03 = strlen(result03->content);
        free(result03);

        // get b again,
        CER_resource_t *result04 = malloc(sizeof (CER_resource_t));
        int depth04 = CEmbedResources_GetResourceByIdentifyer(table, (char *) "D6670C850CD9357B2EE617E4C21CC5EB", result04);
        printf("-- %s (%d) len: %d\n", result04->resourceName, depth04, strlen(result04->content));
        int len04 = strlen(result04->content);
        free(result04);

        CU_ASSERT_TRUE(len01 == len03);
        CU_ASSERT_TRUE(len02 == len04);

    }
    free(table);
}

void testMulitigetDifferentElementByContent() {
    CER_resourcetable_t *table = malloc(sizeof (CER_resourcetable_t));
    CEmbedResources_Init(table, &initialize_resources);
    ...
    for (i = 0; i < 10000; i++) {
        ...
        // get a
        char *char01 = CEmbedResources_GetContentByIdentifyer(table, (char *) "ADAD0000000000000000000000000000");
        printf("-- len: %d\n", strlen(char01));
        int len01 = strlen(char01);
        free(char01);

        // get b
        char *char02 = CEmbedResources_GetContentByIdentifyer(table, (char *) "D6670C850CD9357B2EE617E4C21CC5EB");
        printf("-- len: %d\n", strlen(char02));
        int len02 = strlen(char02);
        free(char02);

        CU_ASSERT_FALSE(len01 == len02);

        // get a again, 
        char *char03 = CEmbedResources_GetContentByIdentifyer(table, (char *) "ADAD0000000000000000000000000000");
        printf("-- len: %d\n", strlen(char03));
        int len03 = strlen(char03);
        free(char03);

        // get b again,
        char *char04 = CEmbedResources_GetContentByIdentifyer(table, (char *) "D6670C850CD9357B2EE617E4C21CC5EB");
        printf("-- len: %d\n", strlen(char04));
        int len04 = strlen(char04);
        free(char04);
...
int main() {
    ...
    if ((NULL == CU_add_test(pSuite, "test initialize", test_initialize)) ||
            ...
            (NULL == CU_add_test(pSuite, "testMulitigetDifferentElementByResource", testMulitigetDifferentElementByResource)) ||
            (NULL == CU_add_test(pSuite, "testMulitigetDifferentElementByContent", testMulitigetDifferentElementByContent)) ||
